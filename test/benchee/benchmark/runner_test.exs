defmodule Benchee.Benchmark.RunnerTest do
  ...
  describe ".run_scenarios" do
    ...
    test "measures the reduction count of a scenario" do
      suite =
        test_suite(%Suite{configuration: %{time: 60_000, warmup: 10_000, reduction_time: 2_000_000}})
      ...
      reduction_counts = hd(new_suite.scenarios).reduction_data.samples

      assert length(reduction_counts) > 0
      assert Enum.all?(reduction_counts, fn count -> count == 550 end)
    end
